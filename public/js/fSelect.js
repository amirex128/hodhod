(function($) {
    ...
    $.fn.fSelect = function(options) {
        ...
        if ('string' === typeof options) {
            ...
            var settings = $.extend({
                placeholder: 'مقدار خود را انتخاب نمایید...',
                numDisplayed: 5,
                overflowText: '{n} مورد انتخاب شده',
                searchText: 'جستجو',
        ...
        fSelect.prototype = {
            create: function() {
                ...
                if (this.settings.showSearch) {
                    search_html = '<div style="border-radius: 5px" class="fs-search"><input style="border-radius: 5px" type="search" placeholder="' + this.settings.searchText + '" /></div>';
                }
                if ('' !== this.settings.noResultsText) {
                    no_results_html = '<div  class="fs-no-results hidden">' + this.settings.noResultsText + '</div>';
                }

                var html = '<div class="fs-label-wrap"><div style="border-radius: 5px" class="fs-label"></div><span class="fs-arrow"></span></div>';
                html += '<div class="fs-dropdown hidden">{search}{no-results}<div style="border-radius: 5px" class="fs-options">' + choices_html + '</div></div>';
                html = html.replace('{search}', search_html);
                html = html.replace('{no-results}', no_results_html);

                this.$select.wrap('<div  class="fs-wrap form-control' + (this.settings.multiple ? ' multiple' : '') + '" tabindex="0" />');
